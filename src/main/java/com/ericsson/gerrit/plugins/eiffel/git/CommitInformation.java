 ...
 *
 * <strong>Note:</strong> This class uses injects. To use this class the caller
 * must inject it, otherwise the injection framework cannot instantiate this
 * class properly.
 *
 */
public class CommitInformation {
	...
	/**
	 * Package private constructor with injection for testing reasons. Injecting
	 * directly as field variables makes it harder to test
	 */
	@Inject
	...
	/**
	 * Fetches a list of parents SHA from a commit. An empty list will be returned
	 * if any error occurs.
	 *
	...
	/**
	 * Will return a list of parents for a given commit. If the collections cannot
	 * find the project or commit, an empty list will be returned.
	 */
	private List<RevCommit> getParents(final String commitId, final String projectName) {
		...
		try {
			...
			final String message = String.format("Error finding the commit for %s in %s", commitId, projectName);
			LOGGER.error(message, e);
		} catch (RestApiException e) {
			final String message = String.format("Error finding the project %s", projectName);
			LOGGER.error(message, e);
		} catch (PermissionBackendException e) {
			final String message = String.format("cannot have permission to this project %s", projectName);
			LOGGER.error(message, e);
		}
		return parents;
	}
	...
	private List<RevCommit> getParentsFromCommit(final String commitId, final String projectName)
		...
		final CommitResource commitResource = commitsCollection.parse(projectResource, IdString.fromDecoded(commitId));
	...
	private List<String> getSHAs(final List<RevCommit> parents) {
		final List<String> parentsSha = parents.stream().map(parent -> parent.getName()).collect(Collectors.toList());
		return parentsSha;
	}
